...
void destroyedProtossShip(Vector *vec, ProtossShips **enemy, int *enemyId ){
  vectorPop(vec); //popvame елемента от вектора 
  *enemyId = vectorGetSize(vec) - 1; //след това последния елемент(последния индекс на корабите на срещуположната флота)
  // трябва да се зададе като вземем горелината на вектора минус последния елемент
